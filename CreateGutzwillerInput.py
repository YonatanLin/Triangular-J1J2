...
def CreateGutzwillerCaseDir(main_results_dir, Lx, Ly, chi):
    Path(main_results_dir).mkdir(parents=True, exist_ok=True)
    gutz_dir = main_results_dir + f"Lx_{Lx}_Ly_{Ly}_chi_{chi}"
...
def CreateGutzwillerCaseDirFromInputFile(main_results_dir, input_file):
    ...
    print(params)
    assert(params[0] == "Lx" and params[1] == "Ly" and params[2] == "chi\n")
    input_for_condor = open("gutz_condor_cases.txt", 'w')
    for line in input_file_lines[1:]:
        params = line.split(" ")
        case_folder = CreateGutzwillerCaseDir(main_results_dir, params[0], params[1], params[2].split("\n")[0])
